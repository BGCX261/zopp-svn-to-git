	...
	$('#content').css({bottom: 11 - bOffset});*/
	  ...
	  var methods = {
            ...
            onConsoleRender: function() {
              ...
              $("#content").css({padding: "" + height + "px 0 10px 0"});
	    ...
	    _scrollBottomWindow: function() {
		  ...
		  //if (!gIsIphone)
				...
				//if ($("#content").height()>440)
					//$("#content").css({bottom: "11px"});
				//$("#t-screen").html = bOffset + "."
	...
	_windowKeydown: function(event) {
	    ...
	    _handleKeyEvent: function(event) {
	      ...
	      var oldPos = self._lineEditor.pos;
			...
			//if (isEnterKeyValue == true){
				isEnterKeyValue = false;
				bOffset = 0;
				var finalInputString = self._lineEditor.acceptLine();
				var callback = self._currentCallback;
				
				if (finalInputString == "#config") {
					
						try {
							Mojo.Controller.StageController.swapScene('title');
							//this.controller.stageController.assistant.showScene("title")
							//this.controller.stageController.popScene();
						} 
						catch (e) {
							Mojo.Log.error(e);
						}
					//self._lineEditor.acceptLine() = "south";
					//$("#t-screen").css({background: black});
					...
					self.current_input.replaceWith(('<span class="finished-input ' + styles + '">' +
					finalInputString.entityify() +
					'</span><br/>'));
				...
				if (event.keyCode in keyCodeHandlerMap) {
    ...
    /*onSave: function(data) {
      ...
      if (b64data) {
		...
		onSave: function(data){
			var saveKey = gStory + '_saveData';
			var b64data = file.base64_encode(data);
			var save_cookie = new Mojo.Model.Cookie(saveKey);
			save_cookie.put(b64data);
			return true;
		},
	  ...
	  $("#top-window").css({width: "900px"});
	  $("#content").css({top: -(self.height) + "px"});
...
function processZcodeAppspotResponse(content) {
  if (content.error)
    throw new FatalError("Error loading story: " + LoadStory +": "+ content.error.entityify());
...
function pInit() {
  ...
  var story = qs.get("story", LoadStory);
  //var story = qs.get("story", LoadStory)
